...
C     temporarily changed 1.d3 to 1.d40  8/17/94 !!!!!!!!!!!!!
        IF(PDAV.GT.1.D40) THEN
          WRITE(*,749)
749   FORMAT(/2X,'****  NOTE LARGE VALUE OF ONE OR MORE RELATIVE STANDAR
     +D DEVIATIONS ****'/,
...
769   FORMAT(/2X,'  #!!!!  BAD CORRELATION MATRIX  !!!!#'
     + /5X,'PROBABLY AT LEAST ONE FREE PARAMETER IS NOT USED IN FITTING
     +FUNCTION'/5X,'OR INITIAL VALUE OF A PARAMETER IS TOO FAR FROM THE
     +CORRECT VALUE'//)
C
763   CONTINUE
        IF(IBAD.GE.1) THEN
